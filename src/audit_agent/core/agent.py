"""Checkpointed and traced event stream for the Audit Agent workflow."""
...
from audit_agent.core.approval import ApprovalHandler
...
from audit_agent.core.state import create_runtime
from audit_agent.core.trace import TraceRecorder
...
def _review_summary_from_state(state: Mapping[str, Any]) -> str:
    """Extract review findings from state and format a compact one-liner."""

    findings = state.get("verified_findings")
    if not isinstance(findings, list) or not findings:
        findings = state.get("review_findings")
    return format_findings_summary(findings if isinstance(findings, list) else None)
...
def _session_assistant_summary(
    ...
    *,
    ...
    return str(
        _review_summary_from_state(state)
