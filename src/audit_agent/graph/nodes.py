"""Core supervisor, auditor coordination, and verifier nodes for Audit Agent."""
...
from audit_agent.agents.auditor import run_auditor
from audit_agent.agents.search_agent import run_search_agent
...
def intent_router_node(state: AuditGraphState) -> dict[str, Any]:
...
def chat_responder_node(state: AuditGraphState) -> dict[str, str]:
...
def intent_route_fn(state: AuditGraphState) -> str:
...
def planner_node(state: AuditGraphState) -> dict[str, Any]:
    ...
    def call_auditors(instruction: str) -> dict[str, Any]:
        return _call_auditors(
            working_state,
            emit,
            instruction,
        )
    ...
    call_audit_tool = StructuredTool.from_function(
        func=call_auditors,
        name="call_auditors",
        description=(
            "Delegate code review to four parallel auditors: security_auditor, "
            "perf_auditor, correctness_auditor, and style_auditor. Each auditor "
            "inspects the codebase with read-only tools and returns structured "
            "findings. Use this after analyzing what should be reviewed."
        ),
        args_schema=SpecialistInstructionInput,
    )
    tools = [todo_write_tool, call_search_tool, call_audit_tool]
    ...
    update: dict[str, Any] = {
        ...
        "review_findings": working_state.get("review_findings", []),
...
def verifier_node(state: AuditGraphState) -> dict[str, Any]:
    """Adversarially verify each auditor finding using read-only tools."""

    runtime = _require_runtime(state)
    runtime.approval_tracker.set_attempt(state.get("attempts", 0) + 1)

    # Optional: run any linter / type-checker commands the planner requested.
                ...
                *messages,
                HumanMessage(
                    content=(
                        ...
                        "any more tools. Based on the evidence already present "
                        "above, return the required verifier JSON object now."
                    )
                ),
            ]
        )
    ...
    # Collect verified findings, falling back to raw review_findings
    verified_findings = decision.verified_findings
    if not verified_findings:
        # If the model didn't return verified_findings, treat raw findings
        # as unverified (no verdict) so downstream can still display them.
        raw = state.get("review_findings", [])
        if isinstance(raw, list) and raw:
            verified_findings = [
                {
                    **f,
                    "verdict": "unverified",
                    "verdict_reason": "Verifier did not return verified_findings.",
                }
                for f in raw
    ...
    commands_passed = all(
        r["ok"] for r in verification_results
    )
    passed = decision.passed and commands_passed
    reason = decision.reason
    if not commands_passed:
        failed_commands = [
            r["command"] for r in verification_results if not r["ok"]
        ]
        reason = (
...
def context_monitor_node(state: AuditGraphState) -> dict[str, Any]:
...
def context_compressor_node(state: AuditGraphState) -> dict[str, Any]:
...
def context_monitor_route(state: AuditGraphState) -> str:
...
def context_compressor_route(state: AuditGraphState) -> str:
    """Resume at the destination saved before context compression."""

    return state.get("context_next_node", "verifier")


def verifier_route(state: AuditGraphState) -> str:
...
def _call_auditors(
    ...
    """Run four audit dimensions and search in parallel, then aggregate findings."""

    dimensions = ["security", "perf", "correctness", "style"]
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for dim in dimensions:
            ...
            futures[future] = dim
        # Run search_agent in parallel for CVE / best-practice lookups
        ...
        futures[search_future] = "search"

        for future in as_completed(futures):
            label = futures[future]
            try:
                result = future.result(timeout=_AUDITOR_TIMEOUT)
            ...
            if label == "search":
                _merge_search_result(state, result)
                continue
            ...
            summaries[label] = str(result.get("summary") or "")
    ...
    # Build one combined handoff record
    summary_parts = [
        f"[{dim}] {summaries.get(dim, 'no output')}"
        for dim in dimensions
    ]
    state["agent_handoffs"] = [
        *state.get("agent_handoffs", []),
...
def _merge_search_result(
    ...
    """Merge search_agent output into the working state."""
    ...
    if notes:
        current = str(state.get("research_notes") or "")
...
def _require_runtime(state: AuditGraphState) -> RuntimeState:
...
def _truncated_context_fields(state: AuditGraphState) -> dict[str, Any]:
    ...
    return {
        ...
        "review_findings": state.get("review_findings", [])[-30:],
        "verified_findings": state.get("verified_findings", [])[-30:],
        "verification_results": verification_results,
        "verification_checks": verification_checks,
