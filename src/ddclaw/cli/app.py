"""Typer entry point for the DDclaw command."""
...
from ddclaw.core.agent import stream_agent_events

app = typer.Typer(
    add_completion=False,
    help="Run a workspace-scoped coding task with DeepSeek.",
...
def main(
    ...
    task: Annotated[
        str | None,
        typer.Argument(
            help="Coding task for DDclaw, optional when --resume is used."
        ),
    ] = None,
    workspace: Annotated[
        Path | None,
        typer.Option(
            ...
            "-w",
            help=(
                "Workspace directory. Defaults to .ddclaw-workspace in the "
                "current directory."
            ),
            file_okay=False,
            dir_okay=True,
    ...
    console.print(
        f"[bold cyan]🚀 DDclaw run started[/bold cyan] · "
        f"workspace: {Path(workspace_path).expanduser().resolve()}"
    )
    console.print("[dim]🧠 Waiting for Planner and DeepSeek events...[/dim]")
    try:
        ...
        error_console.print(
            f"[dim]Resume with: ddclaw --resume "
...
def _render_planner(update: Mapping[str, Any]) -> None:
    ...
    code_summary = update.get("code_agent_summary")
    if code_summary:
        parts.append("\nCode agent summary:")
        parts.append(str(code_summary))
