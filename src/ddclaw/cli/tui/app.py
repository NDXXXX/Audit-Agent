"""Interactive Textual application for multi-turn DDclaw sessions."""
...
from ddclaw.cli.tui.logo import DDClawLogo, LogoState
from ddclaw.core.agent import stream_session_events
from ddclaw.core.approval import ApprovalDecision, ApprovalRequest
from ddclaw.core.session import load_or_create_session
...
class DDClawTuiApp(App[None]):
    """Persistent multi-turn DDclaw terminal interface."""
    ...
    def __init__(
        ...
        *,
        ...
        self.session_workspace = Path(
            session_workspace or Path.cwd() / ".ddclaw-workspace"
        ).expanduser().resolve()
    ...
    def compose(self) -> ComposeResult:
        ...
        yield DDClawLogo(id="ascii-logo")
        yield Static(
    ...
    def _approval_handler(
        ...
        try:
            if not self.post_message(ApprovalRequestedMessage(gate)):
                gate.resolve(False, "The DDclaw TUI is no longer running.")
            return gate.wait()
        finally:
            with self._gate_lock:
    ...
    def on_agent_run_finished_message(
        ...
        if message.error:
            ...
            logo = self.query_one("#ascii-logo", DDClawLogo)
    ...
    def on_approval_requested_message(
        ...
        def resolve(approved: bool | None) -> None:
            ...
            gate.resolve(
                accepted,
                (
                    "Approved by the user in the DDclaw TUI."
                    if accepted
                    else "Denied by the user in the DDclaw TUI."
                ),
            )
    ...
    def _set_logo_state(self, state: LogoState) -> None:
        self.query_one("#ascii-logo", DDClawLogo).set_state(state)
    ...
    def on_unmount(self) -> None:
        ...
        for gate in gates:
            gate.resolve(False, "DDclaw TUI closed before approval was decided.")
...
def run_tui(
    ...
    *,
    ...
    """Create and run the DDclaw Textual application."""
