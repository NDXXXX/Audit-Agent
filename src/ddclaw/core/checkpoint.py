"""Workspace checkpoint persistence and recovery for DDclaw runs."""
...
_CHECKPOINT_DIRECTORY = ".ddclaw/checkpoints"
    ...
    def begin_run(self, run_id: str) -> None:
        ...
        if lease_path.is_file():
            ...
            if _lease_is_active(lease) and lease.get("run_id") != run_id:
                raise RuntimeError(
                    "A DDclaw run is already active in this workspace "
        ...
        *,
        ...
        if snapshot:
            ...
            git_commit = snapshot_workspace_git(
                self.workspace,
                self.root,
                message=f"DDclaw checkpoint: {latest_node or status}",
            )
        else:
...
def snapshot_workspace_git(
    ...
    *,
    ...
    try:
        ...
        _run_git([*command, "config", "user.name", "DDclaw Checkpoint"])
        _run_git(
            [*command, "config", "user.email", "checkpoint@ddclaw.local"]
        )
...
def resume_command(workspace: Path) -> str:
    ...
    return f"ddclaw --resume {shlex.quote(str(workspace.resolve()))}"
...
def _is_internal_path(relative: Path) -> bool:
    return bool(relative.parts) and (
        relative.parts[0] in {".ddclaw", ".git"}
